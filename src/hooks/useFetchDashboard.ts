import {
  ...
  getAdminDashboardMainChart,
  getAdminDashboardRevenueCategory,
  getAdminDashboardRevenueStore,
} from "@/apis/dashboardApi";
import {
  AdminDashboardInfo,
  AdminDashboardMainChart,
  AdminDashboardRevenueCategory,
  AdminDashboardRevenueStore,
...
export const useFetchDashboard = create<DashboardState>((set) => ({
  ...
  fetchDashboard: async (month?: string, year?: string) => {
    ...
    try {
      const [info, mainChart, revenueCategory, revenueStore] =
        await Promise.all([
          getAdminDashboardInfo(),
